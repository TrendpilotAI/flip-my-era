...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  // Find user by email
  const { data: userData, error: userError } = await supabase.from('profiles').select('id').eq('email', customerEmail).single();
  if (userError) {
...
async function allocateCredits(userId: string, creditAmount: number, session: Stripe.Checkout.Session, item: Stripe.LineItem) {
  ...
  const { data: creditData, error: creditError } = await supabase.from('user_credits').select('balance, total_earned').eq('user_id', userId).single();
  let currentBalance = 0;
  let totalEarned = 0;
  if (creditError && creditError.code === 'PGRST116') {
    console.log(`Creating new credit record for user ${userId}`);
    // Create new credit record
    const { data: newCredit, error: createError } = await supabase.from('user_credits').insert({
      user_id: userId,
      balance: creditAmount,
      total_earned: creditAmount,
      total_spent: 0
    }).select('balance, total_earned').single();
    if (createError) {
